...
W tej pracy badane są zalety wykorzystywania danych domeny transformacji falkowej w zadaniu SR, zwłaszcza w celu uchwycenia większej ilości informacji strukturalnych w obrazach aby uniknąć artefaktów. Wykorzystanie różnic współczynników falkowych jako par danych treningowych dodatkowo zwiększa rzadkość danych treningowych, co skutkuje bardziej efektywnym uczeniem. 
% Innymi słowy, użycie współczynników falkowych sprzyja rzadkości aktywacji w warstwach środkowych, a także w warstwie wyjściowej.
...
Dodatkowo transformacja falkowa dekomponuje obraz na wiele skal, co pozwala na wykorzystanie informacji z różnych skal w celu rekonstrukcji obrazu o wysokiej rozdzielczości. \textbf{DWSR} wykorzystuje więc informacje z wielu skal, co pozwala na lepsze odwzorowanie obrazu o wysokiej rozdzielczości.
...
% Sieć \textbf{DWSR} charakteryzuje się krótkim czasem działania, co zapewnia relatywnie niewielka ilość parametrów w porównaniu z innymi metodami SR. Algorytm \textbf{DWSR} również świetnie radzi sobie z rekonstrukcją szczegółów obrazu, zwłaszcza w przypadku stałych krawędzi i tekstur.
