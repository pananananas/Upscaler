...
def generate_color_sr(input_dir, lum_dir, output_dir, scale=2):
    ...
    for img_idx, input_image_path in enumerate(input_images):
        input_image_name = os.path.basename(input_image_path)
        print(f'Image: {img_idx + 1} | {input_image_name}')
        
        # Read bicubic down-scaled image
        bic_test_img = cv2.imread(input_image_path)
        ...
        sr_img = cv2.imread(os.path.join(lum_dir, input_image_name), cv2.IMREAD_GRAYSCALE)
        ...
        cv2.imwrite(os.path.join(output_dir, input_image_name), sr_color)
